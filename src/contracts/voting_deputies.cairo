...
trait VotingABI<TContractState> {
    fn vote(ref self: TContractState, deputie_index: felt252, secret_hash: felt252);
...
mod VotingContract {
    ...
    impl VotingABIImpl of VotingABI<ContractState> {
        fn vote(ref self: ContractState, deputie_index: felt252, secret_hash: felt252) {
            // Verificar que no ha votado antes
            let caller = get_caller_address();
            let already_voted = self.has_voted.read((caller, deputie_index));
            assert(already_voted == 0, 'User already voted');

            // Registrar voto
            self.has_voted.write((caller, deputie_index), 1);

            // Actualizar conteo de votos
            let current_votes = self.proposal_votes.read(deputie_index);
            self.proposal_votes.write(deputie_index, current_votes + 1);
        }
